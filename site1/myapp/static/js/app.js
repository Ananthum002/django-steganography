...
if(coverimage){
    console.log("Filename =", coverimage.name);
    console.log("Size =", coverimage.size);
    console.log("Valid Image =", isImageValid(coverimage.name));
    console.log("Valid Size =", isvalidSize(coverimage.size));
}
        ...
        if(coverimage && isImageValid(coverimage.name) && isvalidSize(coverimage.size)) {
            ...
            if(istextsteg) {
                ...
                formData.append('textfile', $("#fileInput").prop("files")[0]);
    ...
    $('#fileInput').change(function() {
        var textfile = $("#fileInput").prop("files")[0];
...
const decode_image_url = async (image)=>{
    const res =  await fetch(`/myapp/decode_image_url?image=${image}`)
    const data  = await res.json()
    // alert(data.message)
    document.getElementById('#messageBody').innerHTML = data.message;
